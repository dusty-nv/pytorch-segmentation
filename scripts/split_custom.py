...
all_masks = [os.path.splitext(x)[0] for x in os.listdir(INPUT_MASK_PATH) if MASK_FORMAT in os.path.splitext(x)[1]]


all_images = [os.path.splitext(x)[0] for x in os.listdir(INPUT_IMAGE_PATH) if IMAGE_FORMAT in os.path.splitext(x)[1] and os.path.splitext(x)[0] in all_masks]
...
# Split images to train, val and test sets (80% : 20%)
...
print(f'SPLIT: {len(train_images)} train and {len(val_images)} validation images!')
print('-------------------------------------------------------------------------------')
...
print(f'Writing images to the {image_folders[0][1]} and {image_folders[1][1]} folders...')
...
print(f'Writing masks to the {mask_folders[0][1]} and {mask_folders[1][1]} folders...')    
